...
module.exports = (function DEF(ASQ){
  ...
  var brand = "__ASQ__"
  var schedule = ASQ.__schedule
  var tapSequence = ASQ.__tapSequence


  function wrapGate(api,fns,success,failure,reset) {
    fns = map(fns, function $$map(v,idx){
      var def;
      ...
      if (ASQ.isSequence(v)) {
        ...
        return function $$fn(next) {
          def.seq.val(function $$val(){
            success(next,idx,slice(arguments));
          })
          .or(function $$or(){
            failure(next,idx,slice(arguments));
          });
        };
      }
      else {
        return function $$fn(next) {
          ...
          args[0].errfcb = function $$errfcb(err) {
            if (err) {
              ...
              success(next,idx,slice(arguments,1));
            }
          };
...
ASQ.extend("after",function $$extend(api,internals){
  return function $$after(num) {
    ...
    api.then(function $$then(done){
      var args = orig_args || slice(arguments,1);
...
ASQ.after = function $$after() {
  return apply(ASQ().after, ø, arguments);
};
// "any"
    ...
    api.then(function $$then(done){
      ...
      function complete(trigger) {
        if (success_messages.length > 0) {
          ...
          apply(trigger, ø, success_messages);
        }
        else {
          // send errors into main sequence
          error_messages.length = fns.length;
          apply(trigger.fail, ø, error_messages);
        }
      ...
      var completed = 0, error_messages = [], finished = false,
        success_messages = [],
        sq = apply(ASQ, ø, slice(arguments,1))
      ;
  ...
  return function $$errfcb() {
    ...
    return function $$errorfirst$callback(err) {
      if (err) {
        ...
        apply(sq.val_cb, ø, slice(arguments,1));
      }
    };
  };
});
...
ASQ.failAfter = function $$fail$after() {
  return apply(ASQ().failAfter, ø, arguments);
};
// "first"
...
(function IIFE() {
  ...
  ASQ.iterable = function $$iterable() {
    ...
    function val() {
      ...
      var args = slice(arguments).map(function mapper(arg) {
        if (typeof arg != "function") return function $$val() {
          return arg;
        };else return arg;
      });

      apply(val_queue.push, val_queue, args);
    ...
    function pipe() {
      ...
      slice(arguments).forEach(function $$each(fn) {
        val(fn).or(fn.fail);
      });
    ...
    // passed to `val()`
    apply(sequence_api.val, ø, arguments);
...
ASQ.extend("last",function $$extend(api,internals){
  return function $$last() {
    ...
    api.then(function $$then(done){
      ...
      var completed = 0, error_messages = [], finished = false,
        sq = ASQ.apply(ø,slice(arguments,1)),
        success_messages
      ;
    ...
    api.seq(function $$seq(){
      ...
      .gate.apply(ø,arr.map(function $$map(item){
        return function $$segment(){
          each.apply(ø,[item].concat(slice(arguments)));
        };
      }));
    })
    .val(function $$val(){
    ...
    if (typeof success === "function") {
      api.then(function $$then(done){
        if (!ignore_success_handler) {
          ...
          done.apply(ø,slice(arguments,1));
        }
      });
    }
      ...
      api.or(function $$or(){
        if (!ignore_failure_handler) {
          var ret, msgs = ASQ.messages.apply(ø,arguments), smgs,
            or_queue = slice(internals("or_queue"))
          ;
