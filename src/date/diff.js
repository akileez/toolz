...
function diff (start, end, unitName) {
  ...
  if (unitName === 'month') output = getMonthsDiff(start, end)
  else if (unitName === 'year') output = getYearsDiff(start, end)
...
function getElapsedMonths (start, end) {
  ...
  if (start.getDate() > end.getDate()) monthDiff -= 1
...
function getElapsedYears (start, end) {
  ...
  if (start.getMonth() > end.getMonth()) yearDiff -= 1
...
function getFractionalMonth (start, end) {
  ...
  if (startDay !== endDay) {
    var startTotalDays = daysInTheMonth(start)
    var endTotalDays = daysInTheMonth(end)
    var totalDays
    var daysElapsed
    ...
    if (startDay > endDay) {
      ...
      // total days should be relative to 1st day of the next month if
...
function getFractionalYear (start, end, elapsedYears) {
  ...
  return elapsedDays / daysInTheYear(end)
}
